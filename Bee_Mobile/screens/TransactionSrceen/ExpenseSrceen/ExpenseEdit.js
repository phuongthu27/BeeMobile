...
const ExpenseEdit = ({ route }) => {
  ...
  const [selectedDate, setSelectedDate] = useState(moment(expense.date).format('DD/MM/YYYY'));
  const [isModalVisible, setModalVisible] = useState(false);
  const [tempDate, setTempDate] = useState(moment(expense.date).format('YYYY-MM-DD'));
  const scaleValue = new Animated.Value(0);
  

  useEffect(() => {
    const formattedAmount = formatAmount(expense.amount.toString());
  ...
  return (
    <ScrollView contentContainerStyle={styles.card}>
      ...
      <TouchableOpacity style={styles.addButton} onPress={handleUpdateExpense}>
